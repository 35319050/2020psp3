...
## ソースコードの説明
- 表を初期化する
- サイゼリアのメニューを上から1つずつ表において価格の少ないとこから順に以下の処理を行う。
- 価格がオーバーしてしまう場合は上の値を引き継ぐ
- 価格的にメニューが収まる場合は上の値と新たなメニューを入れる前の価格的に最適な状態＋新たなメニューのカロリーを比較し、大きいほうの値をその部分に代入する。
- 最終的に、表の右下の値が求めたいものとなるので、それを出力する。


## 出力結果

```
PS C:\Users\bb35319050\OneDrive - Nagasaki University\ドキュメント\GitHub\2020psp3\k06> & .\"k06.exe"  
max calorie = 1796
```
