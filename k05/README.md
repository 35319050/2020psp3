...
DepthFirstSearch
- 各変数を宣言する
- 訪れたことがあるかどうか示すvisitedを初期化する
- スタックを初期化する
- スタックに出発点のindexをプッシュする
- スタックが空になるまで以下の関数を実行する
- スタックからindexをポップし、valに格納する
- 行ったことがあるかどうかを判別し、行ったことがなければ行ったことにしてその駅名を出力する
- 隣接行列を用いて、ポップした場所から行ける場所をプッシュする
...
- キューを初期化する
- キューに出発点のindexの値を入れる
- キューが空になるまで以下の関数を実行する
- キューからindexの値を取り出し、valに格納する
- 行ったことがあるかどうかを判別し、行ったことがなければ行ったことにしてその駅名を出力する
- 隣接行列を用いて、値をとりだした場所から行ける場所をキューに入れる


## 出力結果

```
PS C:\Users\bb35319050\OneDrive - Nagasaki University\ドキュメント\GitHub\2020psp3\k05> & .\"k05.exe"  
赤迫
長崎大学前
長崎駅前
市民会館
